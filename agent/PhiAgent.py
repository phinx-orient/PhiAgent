...
class PhiAgent(Workflow):
    ...
    async def prepare_chat_history(self, ctx: Context, ev: StartEvent) -> InputEvent:
        ...
        # # save the system_prompt to the context
        # context_msg = ChatMessage(role="system", content=context_str)
        # await ctx.set("context_msg", context_msg)
    ...
    async def handle_llm_input(
        ...
        if not found_answer:
            ctx.write_event_to_stream(ToolStreamEvent(text=str(tool_calls)))
        if not tool_calls:
